jQuery(document).ready(function(){
	...
	var text = $.trim($('#comment_content_id_'+targetId).children('div.padding').children('div.text-style').html());
	if (ls.comments.options.commentId!=targetId && ls.comments.options.commentId != 0){
	    $('#comment_content_id_'+targetId).children('div.padding').children('div.text-style').empty();
	    $('#comment_content_id_'+ls.comments.options.commentId).children('div.padding').children('div.text-style').children('div').appendTo($('#comment_content_id_'+targetId));
	    $('#comment_content_id_'+ls.comments.options.commentId).children('div.padding').children('div.text-style').html(ls.comments.options.text);
	    //$('#comment_content_id_'+targetId).children('div').remove();
	    //$('#form_comment_edit_text').val('');
	} else {
	    $('#comment_content_id_'+targetId).children('div.padding').children('div.text-style').empty();
	    $('#edit-comment').children('div').appendTo($('#comment_content_id_'+targetId).children('div.padding').children('div.text-style'));
	}	   
	...
	$('#comment_content_id_'+ls.comments.options.commentId).children('div.padding').children('div.text-style').children('div').appendTo($('#edit-comment'));
	$('#comment_content_id_'+targetId).children('div.padding').children('div.text-style').html(ls.comments.options.text);
	ls.comments.options.commentId = 0;
	return false;
    };
	...
	ls.ajax(aRouter['ec_ajax']+'savecomment/',{commentId: targetId, text:text, security_ls_key: LIVESTREET_SECURITY_KEY},function(result) {
		...
		ls.comments.shbutton(formObj,targetId,'cancel');
		$('#comment_content_id_'+ls.comments.options.commentId).children('div.padding').children('div.text-style').children('div').appendTo($('#edit-comment'));
		$('#comment_content_id_'+targetId).children('div.padding').children('div.text-style').html(result.sText);
