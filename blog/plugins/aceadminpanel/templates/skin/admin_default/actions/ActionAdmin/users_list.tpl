...
<h3>{$oLang->adm_users_list} <span>({$aStat.count_all})</span></h3>

<ul class="block-nav -box">
    <li {if $sMode=='all' || $sMode==''}class="active"{/if}><strong></strong><a href="{router page='admin'}users/list/">all</a></li>
    <li {if $sMode=='admins'}class="active"{/if}><a href="{router page='admin'}users/admins/">admins</a><em></em></li>
</ul>
...
<table class="admin-table">
    ...
    {if $oConfig->GetValue('general.reg.activation') AND !$oUser->getDateActivate()}
        {assign var=classIcon value='adm_noactive'}
    {elseif $oUser->IsBannedByLogin()}
        {assign var=classIcon value='adm_banned'}
    {elseif $oUser->isAdministrator()}
        {assign var=classIcon value='adm_admin'}
    {else}
        {assign var=classIcon value='adm_user'}
    {/if}
    <tr class="{$className}">
        <td class="center">
        ...
        <td {if $oUserCurrent->GetId()==$oUser->getId()}style="font-weight:bold;"{/if}>
            <div class="{$classIcon}" style="background-image: url({$sWebPluginSkin}/images/adm_icons.png)"></div>
