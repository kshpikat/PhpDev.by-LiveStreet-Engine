		<ul class="menu">
			...
			<li {if $sAction=='profile'}class="active"{/if}>
				...
				{if $sAction=='profile'}
					<ul class="sub-menu" >
						<li {if $aParams[0]=='whois' or $aParams[0]==''}class="active"{/if}><div><a href="{$oUserProfile->getUserWebPath()}">{$aLang.user_menu_profile_whois}</a></div></li>						
						<li {if $aParams[0]=='favourites' and $aParams[1]==''}class="active"{/if}><div><a href="{router page='profile'}{$oUserProfile->getLogin()}/favourites/">{$aLang.user_menu_profile_favourites}</a>{if $iCountTopicFavourite} ({$iCountTopicFavourite}){/if}</div></li>	
